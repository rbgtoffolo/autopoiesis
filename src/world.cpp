...
void World::repulsion(Ball &one, Ball &other)
{
  ...
  if ((newVelocityX * newX + newVelocityY * newY) >= 0)
    return;
  ...
  if (one.xVelocity() < 0.001 && one.yVelocity() < 0.001)
    one.setVelocity(ofVec2f{ofRandom(-5.0f, 5.0f), ofRandom(-5.0f, 5.0f)});
  if (other.xVelocity() < 0.001 && other.yVelocity() < 0.001)
    other.setVelocity(ofVec2f{ofRandom(-5.0f, 5.0f), ofRandom(-5.0f, 5.0f)});
